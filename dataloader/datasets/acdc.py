...
class GenACDC(Dataset):
    ...
    def _load_raw_unlabeled_data(self,
        ...
        for patient_i in volumes:
            ...
            if include_all:
                frames = [f for f in range(im.shape[-1]) if
                          (f > exclude_frames[0] and f < exclude_frames[1]) or f == exclude_frames[0] or f ==
                          exclude_frames[1]]
            else:
                frames = [f for f in range(im.shape[-1]) if
                          f not in exclude_frames and f > exclude_frames[0] and f < exclude_frames[1]]
            for frame in frames:
                subject_idx.append(patient_i)
    ...
    def _crop_same(self,
                   image_list: list,
                   # List of images. Each element should be 4-dimensional, (slice,height,width,channel)
                   ...
                   pad_mode: str = 'edge',
                   # ['edge', 'constant']. 'edge' pads using the values of the edge pixels, 'constant' pads with a constant value
